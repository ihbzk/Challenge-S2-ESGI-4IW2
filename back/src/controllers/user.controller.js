...
exports.login = async (req, res) => {
    try {
        ...
        if (user.isLocked()) {
            return res.status(403).json({ message: `Compte verrouillé, veuillez réessayer ${moment(user.lockUntil).add(-2, 'hours').fromNow()}` });
        }
        ...
        if (daysSinceLastChange >= 60) {
            ...
            const resetTokenExpiry = moment.tz('Europe/Paris').add(1, 'hour').format('YYYY-MM-DD HH:mm:ss');
        ...
        const authToken = jwt.sign({ id: user.id }, "" + process.env.JWT_SECRET, { expiresIn: expiresIn });
...
exports.forgotPassword = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Email de réinitialisation de mot de passe envoyé' });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
};
...
exports.resetPassword = async (req, res) => {
    try {
        ...
        user.lastPasswordModificationDate = moment.tz('Europe/Paris').format('YYYY-MM-DD HH:mm:ss');
...
exports.resetPasswordVerifyToken = async (req, res) => {
    try {
        ...
        if (!user) {
            return res.status(404).json({ message: 'Lien de confirmation invalide ou expiré' });
        }
